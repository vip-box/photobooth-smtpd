...
############
[ -z "$rewrite_to" ] && rewrite_to="None"
useradd -s /bin/false filter
mkdir -p /var/spool/filter
chown filter /var/spool/filter  
postconf -M filter/unix="filter    unix  -       n       n       -       10       pipe flags=Rq user=filter null_sender= argv=/etc/postfix/filter.py \${sender} ${rewrite_to} \${recipient}"
postconf -P "smtp/inet/content_filter=filter:dummy"
